...
WITH source_rows AS (     
SELECT         NULLIF(BTRIM(COALESCE(rkb.internal_order_number::text, '')), '') AS internal_order_number,         rkb.company_name::text AS company_name,         rkb.approval_line_id::text AS source_row_id,         'RKB_ACTUAL'::text AS source_type     
    FROM vw_rkb_planning_lines rkb     WHERE rkb.is_valid_for_metrics       AND NULLIF(BTRIM(COALESCE(rkb.internal_order_number::text, '')), '') IS NOT NULL       AND NOT (             COALESCE(rkb.approval_status::text, '') ILIKE '%cancel%'          OR COALESCE(rkb.normalized_status::text, '') ILIKE '%cancel%'          OR COALESCE(rkb.approval_status::text, '') ILIKE '%reject%'          OR COALESCE(rkb.normalized_status::text, '') ILIKE '%reject%'       )      

    UNION ALL      
SELECT         NULLIF(BTRIM(COALESCE(rop.internal_order_number::text, '')), '') AS internal_order_number,         rop.company_name::text AS company_name,         rop.approval_line_id::text AS source_row_id,         'ROP'::text AS source_type     
    FROM vw_approval_product_line_context rop     WHERE rop.approval_business_type = 'ROP_PROCUREMENT_REQUEST'       AND rop.is_valid_for_metrics       AND NULLIF(BTRIM(COALESCE(rop.internal_order_number::text, '')), '') IS NOT NULL       AND NOT (             COALESCE(rop.approval_status::text, '') ILIKE '%cancel%'          OR COALESCE(rop.normalized_status::text, '') ILIKE '%cancel%'          OR COALESCE(rop.approval_status::text, '') ILIKE '%reject%'          OR COALESCE(rop.normalized_status::text, '') ILIKE '%reject%'       )      
...
ORDER BY src.company_name)         
        FILTER (WHERE NULLIF(BTRIM(COALESCE(src.company_name, '')), '') IS NOT NULL) AS company_name,     COUNT(*) AS source_row_count,     COUNT(*) 
        FILTER (WHERE src.source_type = 'RKB_ACTUAL') AS rkb_actual_row_count,     COUNT(*) 
        FILTER (WHERE src.source_type = 'ROP') AS rop_row_count,     COUNT(*) 
        FILTER (WHERE src.source_type = 'PO') AS po_row_count,     COUNT(DISTINCT src.source_row_id) 
        FILTER (WHERE src.source_type = 'RKB_ACTUAL') AS rkb_actual_count,     COUNT(DISTINCT src.source_row_id) 
        FILTER (WHERE src.source_type = 'ROP') AS rop_count,     COUNT(DISTINCT src.source_row_id) 
        FILTER (WHERE src.source_type = 'PO') AS po_count,     
    CASE 
        WHEN COUNT(*) 
        FILTER (WHERE src.source_type = 'RKB_ACTUAL') > 0  THEN TRUE 
        ELSE FALSE 
    END AS has_rkb_actual,     
    CASE 
        WHEN COUNT(*) 
        FILTER (WHERE src.source_type = 'ROP') > 0  THEN TRUE 
        ELSE FALSE 
    END AS has_rop,     
    CASE 
        WHEN COUNT(*) 
...
WITH source_rows AS (     
SELECT         NULLIF(BTRIM(COALESCE(rkb.internal_order_number::text, '')), '') AS internal_order_number,         COALESCE(             NULLIF(BTRIM((regexp_match(COALESCE(rkb.product_name::text, ''), '^\[([^\]]+)\]'))[1]), ''),             NULLIF(LOWER(REGEXP_REPLACE(BTRIM(COALESCE(rkb.product_name::text, '')), '[[:space:]]+', ' ', 'g')), '')         ) AS product_key,         NULLIF(BTRIM(COALESCE(rkb.product_name::text, '')), '') AS product_name,         NULLIF(BTRIM(COALESCE(rkb.unit_of_measure::text, '')), '') AS unit_of_measure,         COALESCE(rkb.planned_quantity, 0)::numeric AS qty,         COALESCE(rkb.planned_subtotal, 0)::numeric AS subtotal,         rkb.date_of_need,         rkb.approval_line_id,         NULLIF(BTRIM(COALESCE(rkb.approval_request_id::text, '')), '') AS approval_request_id,         NULLIF(BTRIM(COALESCE(rkb.approval_request_numeric_id::text, '')), '') AS approval_request_numeric_id,         NULLIF(BTRIM(COALESCE(rkb.approval_status::text, '')), '') AS approval_status,         NULLIF(BTRIM(COALESCE(rkb.normalized_status::text, '')), '') AS normalized_status     
    FROM vw_rkb_planning_lines rkb     WHERE rkb.is_valid_for_metrics       AND NULLIF(BTRIM(COALESCE(rkb.internal_order_number::text, '')), '') IS NOT NULL       AND NULLIF(BTRIM(COALESCE(rkb.product_name::text, '')), '') IS NOT NULL       AND NOT (             COALESCE(rkb.approval_status::text, '') ILIKE '%cancel%'          OR COALESCE(rkb.normalized_status::text, '') ILIKE '%cancel%'          OR COALESCE(rkb.approval_status::text, '') ILIKE '%reject%'          OR COALESCE(rkb.normalized_status::text, '') ILIKE '%reject%'       ) )

SELECT     source_rows.internal_order_number,     source_rows.product_key,     MAX(source_rows.product_name) AS product_name,     STRING_AGG(DISTINCT source_rows.unit_of_measure, ', ' 
ORDER BY source_rows.unit_of_measure)         
        FILTER (WHERE NULLIF(BTRIM(COALESCE(source_rows.unit_of_measure, '')), '') IS NOT NULL) AS rkb_actual_uom_summary,     COUNT(DISTINCT NULLIF(BTRIM(COALESCE(source_rows.unit_of_measure, '')), '')) AS rkb_actual_uom_count,     COALESCE(SUM(source_rows.qty), 0)::numeric AS rkb_actual_qty,     
    CASE 
        WHEN COALESCE(SUM(source_rows.qty), 0) <> 0           THEN COALESCE(SUM(source_rows.subtotal), 0)::numeric / NULLIF(COALESCE(SUM(source_rows.qty), 0), 0)          
        ELSE NULL     
    END AS rkb_actual_unit_price,     COALESCE(SUM(source_rows.subtotal), 0)::numeric AS rkb_actual_subtotal,     COUNT(DISTINCT source_rows.approval_line_id) AS rkb_actual_line_count,     STRING_AGG(DISTINCT source_rows.approval_status, ', ' 
ORDER BY source_rows.approval_status)         
        FILTER (WHERE NULLIF(BTRIM(COALESCE(source_rows.approval_status, '')), '') IS NOT NULL) AS rkb_actual_status_summary,     STRING_AGG(DISTINCT source_rows.normalized_status, ', ' 
ORDER BY source_rows.normalized_status)         
        FILTER (WHERE NULLIF(BTRIM(COALESCE(source_rows.normalized_status, '')), '') IS NOT NULL) AS rkb_actual_normalized_status_summary,     STRING_AGG(DISTINCT source_rows.approval_request_id, ', ' 
ORDER BY source_rows.approval_request_id)         
        FILTER (WHERE NULLIF(BTRIM(COALESCE(source_rows.approval_request_id, '')), '') IS NOT NULL) AS rkb_actual_request_summary,     STRING_AGG(DISTINCT source_rows.approval_request_numeric_id, ', ' 
ORDER BY source_rows.approval_request_numeric_id)         
        FILTER (WHERE NULLIF(BTRIM(COALESCE(source_rows.approval_request_numeric_id, '')), '') IS NOT NULL) AS rkb_actual_request_numeric_summary,     MIN(source_rows.date_of_need) AS rkb_actual_date_of_need_min,     MAX(source_rows.date_of_need) AS rkb_actual_date_of_need_max 
FROM source_rows 

GROUP BY source_rows.internal_order_number, source_rows.product_key;

COMMENT ON VIEW vw_internal_order_rkb_actual_agg IS     'Aggregates current/latest Odoo RKB Actual lines by internal_order_number + product_key. Baseline only, not profitability or stock valuation.';

CREATE VIEW vw_internal_order_rop_agg AS 
WITH source_rows AS (     
SELECT         NULLIF(BTRIM(COALESCE(rop.internal_order_number::text, '')), '') AS internal_order_number,         COALESCE(             NULLIF(BTRIM((regexp_match(COALESCE(rop.product_name::text, ''), '^\[([^\]]+)\]'))[1]), ''),             NULLIF(LOWER(REGEXP_REPLACE(BTRIM(COALESCE(rop.product_name::text, '')), '[[:space:]]+', ' ', 'g')), '')         ) AS product_key,         NULLIF(BTRIM(COALESCE(rop.product_name::text, '')), '') AS product_name,         NULLIF(BTRIM(COALESCE(rop.unit_of_measure::text, '')), '') AS unit_of_measure,         COALESCE(rop.planned_quantity, 0)::numeric AS qty,         COALESCE(rop.planned_subtotal, 0)::numeric AS subtotal,         rop.date_of_need,         rop.approval_line_id,         NULLIF(BTRIM(COALESCE(rop.approval_request_id::text, '')), '') AS approval_request_id,         NULLIF(BTRIM(COALESCE(rop.approval_request_numeric_id::text, '')), '') AS approval_request_numeric_id,         NULLIF(BTRIM(COALESCE(rop.approval_status::text, '')), '') AS approval_status,         NULLIF(BTRIM(COALESCE(rop.normalized_status::text, '')), '') AS normalized_status,         NULLIF(BTRIM(COALESCE(rop.approval_category_raw::text, '')), '') AS approval_category_raw,         NULLIF(BTRIM(COALESCE(rop.approval_business_type::text, '')), '') AS approval_business_type     
    FROM vw_approval_product_line_context rop     WHERE rop.approval_business_type = 'ROP_PROCUREMENT_REQUEST'       AND rop.is_valid_for_metrics       AND NULLIF(BTRIM(COALESCE(rop.internal_order_number::text, '')), '') IS NOT NULL       AND NULLIF(BTRIM(COALESCE(rop.product_name::text, '')), '') IS NOT NULL       AND NOT (             COALESCE(rop.approval_status::text, '') ILIKE '%cancel%'          OR COALESCE(rop.normalized_status::text, '') ILIKE '%cancel%'          OR COALESCE(rop.approval_status::text, '') ILIKE '%reject%'          OR COALESCE(rop.normalized_status::text, '') ILIKE '%reject%'       ) )

SELECT     source_rows.internal_order_number,     source_rows.product_key,     MAX(source_rows.product_name) AS product_name,     STRING_AGG(DISTINCT source_rows.unit_of_measure, ', ' 
ORDER BY source_rows.unit_of_measure)         
        FILTER (WHERE NULLIF(BTRIM(COALESCE(source_rows.unit_of_measure, '')), '') IS NOT NULL) AS rop_uom_summary,     COUNT(DISTINCT NULLIF(BTRIM(COALESCE(source_rows.unit_of_measure, '')), '')) AS rop_uom_count,     COALESCE(SUM(source_rows.qty), 0)::numeric AS rop_qty,     
    CASE 
        WHEN COALESCE(SUM(source_rows.qty), 0) <> 0           THEN COALESCE(SUM(source_rows.subtotal), 0)::numeric / NULLIF(COALESCE(SUM(source_rows.qty), 0), 0)          
        ELSE NULL     
    END AS rop_unit_price,     COALESCE(SUM(source_rows.subtotal), 0)::numeric AS rop_subtotal,     COUNT(DISTINCT source_rows.approval_line_id) AS rop_line_count,     MIN(source_rows.date_of_need) AS rop_date_of_need_min,     MAX(source_rows.date_of_need) AS rop_date_of_need_max,     STRING_AGG(DISTINCT source_rows.approval_status, ', ' 
ORDER BY source_rows.approval_status)         
        FILTER (WHERE NULLIF(BTRIM(COALESCE(source_rows.approval_status, '')), '') IS NOT NULL) AS rop_status_summary,     STRING_AGG(DISTINCT source_rows.normalized_status, ', ' 
ORDER BY source_rows.normalized_status)         
        FILTER (WHERE NULLIF(BTRIM(COALESCE(source_rows.normalized_status, '')), '') IS NOT NULL) AS rop_normalized_status_summary,     STRING_AGG(DISTINCT source_rows.approval_category_raw, ', ' 
ORDER BY source_rows.approval_category_raw)         
        FILTER (WHERE NULLIF(BTRIM(COALESCE(source_rows.approval_category_raw, '')), '') IS NOT NULL) AS rop_category_summary,     STRING_AGG(DISTINCT source_rows.approval_request_id, ', ' 
ORDER BY source_rows.approval_request_id)         
        FILTER (WHERE NULLIF(BTRIM(COALESCE(source_rows.approval_request_id, '')), '') IS NOT NULL) AS rop_request_summary,     STRING_AGG(DISTINCT source_rows.approval_request_numeric_id, ', ' 
ORDER BY source_rows.approval_request_numeric_id)         
...
COMMENT ON VIEW vw_internal_order_rop_agg IS     'Aggregates ROP / PEMBELIAN approval lines by internal_order_number + product_key. ROP and PEMBELIAN have the same business meaning.';

CREATE VIEW vw_internal_order_po_agg AS 
WITH source_rows AS (     
SELECT         NULLIF(BTRIM(COALESCE(po.internal_order_number::text, '')), '') AS internal_order_number,         COALESCE(             NULLIF(BTRIM((regexp_match(COALESCE(po.product_name::text, ''), '^\[([^\]]+)\]'))[1]), ''),             NULLIF(LOWER(REGEXP_REPLACE(BTRIM(COALESCE(po.product_name::text, '')), '[[:space:]]+', ' ', 'g')), '')         ) AS product_key,         NULLIF(BTRIM(COALESCE(po.product_name::text, '')), '') AS product_name,         NULLIF(BTRIM(COALESCE(po.unit_of_measure::text, '')), '') AS unit_of_measure,         COALESCE(po.ordered_quantity, 0)::numeric AS ordered_quantity,         COALESCE(po.received_quantity, 0)::numeric AS received_quantity,         COALESCE(po.invoiced_quantity, 0)::numeric AS invoiced_quantity,         COALESCE(po.unit_price, 0)::numeric AS unit_price,         COALESCE(po.line_subtotal, 0)::numeric AS line_subtotal,         po.purchase_planned_date,         NULLIF(BTRIM(COALESCE(po.purchase_line_state::text, '')), '') AS purchase_line_state,         po.procurement_line_id,         NULLIF(BTRIM(COALESCE(po.purchase_order_reference::text, '')), '') AS purchase_order_reference     
...
SELECT     source_rows.internal_order_number,     source_rows.product_key,     MAX(source_rows.product_name) AS product_name,     STRING_AGG(DISTINCT source_rows.unit_of_measure, ', ' 
ORDER BY source_rows.unit_of_measure)         
        FILTER (WHERE NULLIF(BTRIM(COALESCE(source_rows.unit_of_measure, '')), '') IS NOT NULL) AS po_uom_summary,     COUNT(DISTINCT NULLIF(BTRIM(COALESCE(source_rows.unit_of_measure, '')), '')) AS po_uom_count,     COALESCE(SUM(source_rows.ordered_quantity), 0)::numeric AS po_qty,     COALESCE(SUM(source_rows.received_quantity), 0)::numeric AS po_received_qty,     COALESCE(SUM(source_rows.invoiced_quantity), 0)::numeric AS po_invoiced_qty,     
    CASE 
        WHEN COALESCE(SUM(source_rows.ordered_quantity), 0) <> 0           THEN COALESCE(SUM(source_rows.line_subtotal), 0)::numeric / NULLIF(COALESCE(SUM(source_rows.ordered_quantity), 0), 0)          
        ELSE NULL     
    END AS po_unit_price,     COALESCE(SUM(source_rows.line_subtotal), 0)::numeric AS po_subtotal,     COUNT(DISTINCT source_rows.procurement_line_id) AS po_line_count,     MIN(source_rows.purchase_planned_date) AS po_expected_arrival_min,     MAX(source_rows.purchase_planned_date) AS po_expected_arrival_max,     STRING_AGG(DISTINCT source_rows.purchase_line_state, ', ' 
ORDER BY source_rows.purchase_line_state)         
        FILTER (WHERE NULLIF(BTRIM(COALESCE(source_rows.purchase_line_state, '')), '') IS NOT NULL) AS po_status_summary,     STRING_AGG(DISTINCT source_rows.purchase_order_reference, ', ' 
ORDER BY source_rows.purchase_order_reference)         
        FILTER (WHERE NULLIF(BTRIM(COALESCE(source_rows.purchase_order_reference, '')), '') IS NOT NULL) AS po_order_reference_summary 
FROM source_rows 

GROUP BY source_rows.internal_order_number, source_rows.product_key;

COMMENT ON VIEW vw_internal_order_po_agg IS     'Aggregates PO execution by internal_order_number + product_key. This is procurement comparison only, not stock valuation or accounting profit.';

CREATE VIEW vw_internal_order_product_universe AS
...
SELECT
    ...
    STRING_AGG(DISTINCT classified.source_uom_summary, ' ; ' 
ORDER BY classified.source_uom_summary)
    ...
    CASE
        ...
        WHEN MAX(classified.product_trackability_class) = 'TRACKABLE_PRODUCT'  THEN TRUE
...
COMMENT ON VIEW vw_internal_order_product_universe IS     'Conservative product universe for Internal Order Rekap. Exact bracketed product code is preferred; normalized product name is the fallback.';

CREATE VIEW vw_internal_order_rekap_lines AS
...
COMMENT ON VIEW vw_internal_order_rekap_lines IS     'Phase 1 Internal Order Rekap lines at internal_order_number + product_key grain. Quantity comparison remains UoM-sensitive and non-converted.';

CREATE VIEW vw_internal_order_rekap_summary AS
...
SELECT
    ...
    COUNT(*) 
        FILTER (WHERE COALESCE(lines.rop_qty, 0) > 0) AS rop_product_count,
    COUNT(*) 
    ...
    CASE 
        ...
        ELSE 0 END)::numeric AS rkb_actual_trackable_amount,
    SUM(
    CASE 
        WHEN lines.product_trackability_class = 'NON_TRACKABLE_OTHERS'  THEN lines.rkb_actual_subtotal 
        ELSE 0 END)::numeric AS rkb_actual_non_trackable_amount,
    SUM(
    CASE 
        WHEN lines.product_trackability_class = 'UNKNOWN_PRODUCT_CLASS'  THEN lines.rkb_actual_subtotal 
        ELSE 0 END)::numeric AS rkb_actual_unknown_class_amount,
    COUNT(*) 
        FILTER (WHERE COALESCE(lines.rkb_actual_qty, 0) > 0 AND lines.product_trackability_class = 'TRACKABLE_PRODUCT') AS rkb_actual_trackable_product_count,
    COUNT(*) 
        FILTER (WHERE COALESCE(lines.rkb_actual_qty, 0) > 0 AND lines.product_trackability_class = 'NON_TRACKABLE_OTHERS') AS rkb_actual_non_trackable_product_count,
    SUM(lines.rop_subtotal)::numeric AS rop_amount,
    SUM(
    CASE 
        WHEN lines.product_trackability_class = 'TRACKABLE_PRODUCT'  THEN lines.rop_subtotal 
        ELSE 0 END)::numeric AS rop_trackable_amount,
    SUM(
    CASE 
        WHEN lines.product_trackability_class = 'NON_TRACKABLE_OTHERS'  THEN lines.rop_subtotal 
        ELSE 0 END)::numeric AS rop_non_trackable_amount,
    SUM(
    CASE 
        WHEN lines.product_trackability_class = 'UNKNOWN_PRODUCT_CLASS'  THEN lines.rop_subtotal 
        ELSE 0 END)::numeric AS rop_unknown_class_amount,
    SUM(lines.po_subtotal)::numeric AS po_amount,
    SUM(
    CASE 
        WHEN lines.product_trackability_class = 'TRACKABLE_PRODUCT'  THEN lines.po_subtotal 
        ELSE 0 END)::numeric AS po_trackable_amount,
    SUM(
    CASE 
        WHEN lines.product_trackability_class = 'NON_TRACKABLE_OTHERS'  THEN lines.po_subtotal 
        ELSE 0 END)::numeric AS po_non_trackable_amount,
    SUM(
    CASE 
        WHEN lines.product_trackability_class = 'UNKNOWN_PRODUCT_CLASS'  THEN lines.po_subtotal 
        ELSE 0 END)::numeric AS po_unknown_class_amount,
    ...
    SUM(
    CASE 
        WHEN lines.mixed_uom_flag  THEN 1 
        ELSE 0 END) AS mixed_uom_count,
    SUM(
    CASE 
        WHEN lines.product_presence_status = 'RKB_ONLY'  THEN 1 
        ELSE 0 END) AS rkb_only_count,
    SUM(
    CASE 
        WHEN lines.product_presence_status = 'ROP_ONLY'  THEN 1 
        ELSE 0 END) AS rop_only_count,
    SUM(
    CASE 
        WHEN lines.product_presence_status = 'PO_ONLY'  THEN 1 
        ELSE 0 END) AS po_only_count,
    SUM(
    CASE 
        WHEN lines.product_presence_status = 'RKB_ROP_PO'  THEN 1 
        ELSE 0 END) AS rkb_rop_po_count,
    SUM(
    CASE 
        WHEN lines.po_without_rop_flag  THEN 1 
        ELSE 0 END) AS po_without_rop_count,
    SUM(
    CASE 
        WHEN lines.rop_without_po_flag  THEN 1 
        ELSE 0 END) AS rop_without_po_count,
