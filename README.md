...
#### Incremental Sync Re-fetches Records

**Symptom:** Running incremental sync twice fetches the same records.

**Possible Causes:**
1. **Checkpoint not saved** - Check if sync crashed before completion
2. **Boolean/FALSE write_date** - Some models have `write_date=False` instead of datetime
3. **Type mismatch** - Comparing different types (bool vs string)

**Diagnosis:** Look for these log entries:
```
CHECKPOINT_SAVED
...
SYNC STATE READ
  last_sync_date_read=2026-06-19 09:52:36  # Should match saved value
  last_sync_id_read=3910                    # Should match saved value
```

**Solutions:**
- Ensure no `' > ' not supported between instances of 'bool' and 'str'` errors
- Check `sync_state` table in PostgreSQL for NULL values

---

#### Full Sync Every Time

**Symptom:** Incremental sync acts like full sync every run.

**Possible Causes:**
1. `sync_state` has `status=failed` or `last_sync_date=NULL`
2. Model lacks `write_date` field
...
```sql
SELECT model_name, status, last_sync_date, last_sync_id 
FROM sync_state 
...
#### Schema Errors (NULL_CONSTRAINT)

**Symptom:** Records fail with NULL constraint violations.

**Root Cause:** Odoo field marked as `required=True` but PostgreSQL column is nullable.

**Auto-fix:** The sync engine automatically adds `ALTER COLUMN DROP NOT NULL` for problematic columns.
...
**Symptom:** `AuthenticationError` or connection refused.
...
2. Check Odoo API key has correct permissions:
   - Go to Odoo Settings → User & Companies → Users
   - Select user → Edit → Preferences → API Key
   - Generate new key if expired

3. Verify network access to Odoo instance

---

#### Missing Fields in PostgreSQL

**Symptom:** New Odoo fields not appearing in sync.

**Solution:**
```bash
# Run full sync to auto-discover new fields
python -m src.main --mode full --models your.model
```
...
**Symptom:** `psycopg2.errors` or connection timeout.
...
| Log Entry | Meaning |
|-----------|---------|
| `INCREMENTAL FILTER GENERATED` | Incremental domain created |
| `CHECKPOINT_SAVED` | Checkpoint stored in DB |
| `SYNC STATE READ` | Checkpoint loaded from DB |
| `MAX_WRITE_DATE_FOUND` | New max write_date discovered |
| `RECORD_FALSE_WRITE_DATE` | Model has boolean write_date |
| `WRITE_DATE_COMPARISON_ERROR` | Type mismatch in comparison |
