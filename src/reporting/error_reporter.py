...
class DataProfile:
    ...
    def to_dict(self) -> dict:
        return {
            ...
            "max_value_observed": float(self.max_value_observed) if self.max_value_observed else None,
...
class ModelErrorStats:
    ...
    def add_error(self, category: ErrorCategory, record_id: Optional[int] = None,
        ...
        self.processed += 1
        if column_name and not is_cascade:
            self.errors_by_column[column_name] += 1
        if is_cascade:
    ...
    def add_data_profile(self, column_name: str, col_type: str, value: Any):
        """Add a data profile for a column."""
        if column_name not in self.data_profiles:
            self.data_profiles[column_name] = DataProfile(
                column_name=column_name,
                current_type=col_type,
            )
        profile = self.data_profiles[column_name]
        if value is None:
            profile.null_count += 1
        elif isinstance(value, str):
            length = len(value)
            if profile.max_length_observed is None or length > profile.max_length_observed:
                profile.max_length_observed = length
            if len(profile.sample_values) < 5:
                profile.sample_values.append(value[:100])
        elif isinstance(value, (int, float, Decimal)):
            num_val = Decimal(str(value))
            if profile.max_value_observed is None or abs(num_val) > abs(profile.max_value_observed):
                profile.max_value_observed = num_val
            if len(profile.sample_values) < 5:
...
class ErrorReporter:
    ...
    def profile_data(self, column_name: str, column_type: str, value: Any):
        """Profile data values for schema recommendations."""
        if self._current_model:
            stats = self.get_or_create_stats(self._current_model, self._current_table)
            stats.add_data_profile(column_name, column_type, value)
    ...
    def get_summary(self) -> dict:
        ...
        return {
            "models": {model: {"processed": stats.processed, "success": stats.success,
                               ...
                               "errors_by_column": dict(stats.errors_by_column),
                               "data_profiles": {k: v.to_dict() for k, v in stats.data_profiles.items()}}
