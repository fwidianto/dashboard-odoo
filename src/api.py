...
async def get_status():
    """
    Get synchronization status for all models.
    
    Returns current sync state for each configured model,
    including last sync date, record count, and status.
    """
    try:
...
@app.post("/sync/{model_name}", response_model=SyncResponse)
...
@app.post("/reset", response_model=SyncResponse)
...
async def list_models():
    ...
    PostgreSQL table name, and field mappings.
    """
    try:
        engine = get_sync_engine()
